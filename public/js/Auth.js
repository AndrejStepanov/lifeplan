...
/* harmony default export */ __webpack_exports__["default"] = ({
	...
	computed: {
		colorForm: function colorForm() {
			return !this.bottomNav ? 'primary' : 'accent';
		},
	...
	methods: {
		dialogSave: function dialogSave() {
			...
			todo = _extends({}, vm.paramTodo(vm.paramForm));
			for (name in todo) {
				if (!this.bottomNav && ['_login', '_password'].indexOf(name) != -1) tmp[name.slice(1)] = todo[name].value;else tmp[name] = todo[name].value;
			}todo = tmp;
			if (!vm.bottomNav) sendRequest({ href: "/login", type: "auth.login", data: todo, default: getErrDesc('withLogIn') });else sendRequest({ href: "/register", type: "register", needSucess: "Y", data: todo, default: getErrDesc('withRegistration') });
		}
	},
	created: function created() {
		var vm = this;
		...
		vm.$root.$on('dialog' + vm.paramForm + 'Send', function () {
			vm.dialogSave();
		});
	},
	mounted: function mounted() {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__helpers_icon__ = __webpack_require__(102);
//
//
//
//
